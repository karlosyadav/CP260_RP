...
def _load_owlvit():
    if _owlvit_state["model"] is None:
        ...
        mdl = Owlv2ForObjectDetection.from_pretrained(name).to(_device()).eval()
...
# Detection
# -------------------------------------------------------------------------
def _detect_in_frame(rgb_uint8: np.ndarray,
                     prompt: str,
                     thresh: float) -> List[Tuple[Tuple[int, int, int, int], float]]:
    import torch
    from PIL import Image
    proc, mdl = _load_owlvit()
    pil = Image.fromarray(rgb_uint8)
    with torch.no_grad():
        inputs = proc(images=pil, text=[[prompt]], return_tensors="pt").to(_device())
        outputs = mdl(**inputs)
        target_size = torch.tensor([pil.size[::-1]]).to(_device())
        results = proc.post_process_object_detection(
            outputs=outputs, target_sizes=target_size, threshold=thresh,
        )[0]
...
# Segmentation
# -------------------------------------------------------------------------
def _segment_with_box(rgb_uint8: np.ndarray,
                      box: Tuple[int, int, int, int],
                      sam_ckpt: str) -> np.ndarray:
...
# Public API
# -------------------------------------------------------------------------
def segment_entity(entity: str,
                   prompt: str,
                   images: Dict[int, np.ndarray],
                   sam_ckpt: str,
                   thresh: float = S.DETECTOR_THRESHOLD,
                   keep_top_k: int = 1) -> List[MaskHit]:
    """
    ...
    for fid, img in images.items():
        ...
        detections = _detect_in_frame(rgb_uint8, prompt, thresh)
        if not detections:
            continue
        # Use the highest-confidence box from this frame
        detections.sort(key=lambda x: x[1], reverse=True)
        for box, score in detections[:keep_top_k]:
            try:
                mask = _segment_with_box(rgb_uint8, box, sam_ckpt)
            except Exception as exc:                       # pragma: no cover
                print(f"  [warn] SAM failed on frame {fid}: {exc}")
                continue
            if mask.sum() < S.SEG_MIN_AREA:
                continue
            hits.append(MaskHit(box, score, mask, fid))

    hits.sort(key=lambda h: h.score, reverse=True)
    return hits


def segment_all(images: Dict[int, np.ndarray],
                sam_ckpt: str,
    ...
    for entity, prompt in prompts.items():
        ...
        out[entity] = segment_entity(entity, prompt, images, sam_ckpt)
        print(f"     → {len(out[entity])} frames with detection")
    return out
